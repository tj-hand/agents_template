...
## Role
Code review, testing strategy, and quality assurance. Approves or rejects PRs based on quality standards.
...
### Review Workflow
```
1. Git Agent assigns PR → QA Agent
2. QA reviews code + runs tests
3. QA decides:
   - APPROVE → Git Agent merges
   - REQUEST CHANGES → back to developer
   - BLOCK → critical issues found
4. Git Agent updates issue/PR status
```
...
**General (all code):**
- [ ] Issue number referenced in commits (#N)
...
Git Agent: merge PR #78"
```

### REQUEST CHANGES (fixable issues)
```
"QA Agent [CHANGES REQUESTED #45]:
...
**With Git Agent:**
- Git assigns PR review
- QA approves/rejects
- Git merges or requests changes
...
```
Git Agent: "QA Agent [REVIEW]: review PR #78 for issue #45"

Actions:
1. Validate issue #45 exists
2. Check PR against code review checklist
...
7. Comment on PR with decision
8. Signal Git Agent with result
```

**Layer 2 validation:** Issue exists? If NO → STOP
...
1. **Every PR reviewed** - no direct merges to main
...
5. **Issue required** - all PRs reference issue
